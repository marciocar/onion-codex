...
- **`clickup`** → comentário em formatação Unicode via `@clickup-specialist`. Adapter: `.agents/skills/task-manager/references/adapters/clickup.md`. Abstração MCP de referência: `commentPRCreated()` em `.codex/utils/clickup-mcp-wrappers.md` (linhas 632-661). Padrões: `docs/onion/shared/clickup-patterns.md`.
