...
- **Implementação de referência real**: [`task-manager-abstraction.md`](../../../docs/knowledge-base/concepts/task-manager-abstraction.md) e `.agents/skills/task-manager/references/`
...
if [ -d ".codex/utils/{{abstraction_name}}" ]; then
  echo "❌ ERRO: Abstração '{{abstraction_name}}' já existe!"
  ls -la .codex/utils/{{abstraction_name}}/
  exit 1
fi
...
- [ ] Nome único (não existe em `.agents/skills/task-manager/references/`)
...
```bash
mkdir -p .codex/utils/{{abstraction_name}}/adapters
```
...
∟ Exemplo real: .agents/skills/task-manager/references/
...
- [Task Manager (referência)](../../../.agents/skills/task-manager/references/)
- Agente: @onion
