...
description: >
  ...
  sobre por onde começar, qual skill ou subagente usar, como executar um fluxo de trabalho
...
Sessões abertas:
!`ls .codex/sessions/ 2>/dev/null || echo "(nenhuma sessão ativa)"`
...
> Skills são invocadas por `$skill-slug` (ou `/skills`); subagentes por menção explícita (`@nome`).

### Desenvolvimento de Feature
| Intenção | Skill / Subagente |
|----------|-----------------|
...
| Intenção | Skill |
|----------|---------|
| Novo subagente especializado | `$meta-create-agent` |
| Nova skill | `$meta-create-skill` |
| Novo comando/skill | `$meta-create-command` |
...
| Provider | Subagente | Quando usar |
|----------|--------|-------------|
...
## Subagentes por Categoria

**development/** (20): `@react-developer`, `@nodejs-specialist`, `@clickup-specialist`, `@jira-specialist`, `@docker-specialist`, `@codex-specialist`, `@c4-architecture-specialist`, `@mermaid-specialist`, `@nx-monorepo-specialist`, `@zen-engine-specialist` e outros.
...
Correto: `user-authentication`. Errado: `user_authentication`, `UserAuth`, `userAuth`. O slug é usado tanto no nome da branch Git quanto na pasta de sessão `.codex/sessions/<feature-slug>/`.

**Sessões de contexto**
Contexto persistente de feature em `.codex/sessions/<feature-slug>/` com `context.md`, `plan.md`, `architecture.md`. `$engineer-start` cria; `$engineer-work` consume. Verificar se sessão existe antes de recomendar `$engineer-work`.
...
- `$nome` → skill (`.agents/skills/`) — workflows reutilizáveis, invocação explícita ou implícita por descrição
- `@nome` → subagente (`.codex/agents/*.toml`) — delegação especializada, spawn explícito
...
- Subagente orquestrador completo: `.codex/agents/onion.toml`
- Task Manager Abstraction: `docs/knowledge-base/concepts/task-manager-abstraction.md`
- Índice geral: `docs/INDEX.md`
- Guia de skills: `docs/onion/commands-guide.md`
- Referência de subagentes: `docs/onion/agents-reference.md`
