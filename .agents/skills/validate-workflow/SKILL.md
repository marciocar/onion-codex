...
function validateSessionManagement() {
  ...
  if [[ -n "$session_name" ]]; then
    ...
    [[ ! -d ".codex/sessions/$session_name" ]] && 
    [[ -d ".codex/sessions/archived" ]] && 
    ls .codex/sessions/archived/ | grep -q "$session_name"
  else
    # Verificar se há sessões ativas desnecessárias
    active_sessions=$(find .codex/sessions -maxdepth 1 -type d ! -name "sessions" ! -name "archived" | wc -l)
...
function validateCompliance() {
  ...
  [[ -f ".agents/skills/git/sync.md" ]] && 
  grep -q "Branch Protection" ".agents/skills/git/sync.md"
}
