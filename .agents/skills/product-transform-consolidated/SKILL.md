...
> **Convenções de comunicação Codex, nomenclatura e formatação:** seguir a skill `onion-patterns`.
...
Salvar em `.codex/sessions/consolidated-transform/analysis-<timestamp>.yaml`.
...
```bash
OUTPUT_DIR=".codex/sessions/consolidated-transform/"; mkdir -p "$OUTPUT_DIR"
```
