...
**None/Dry-run:** Salvar estrutura local em `.codex/sessions/test-strategies/[feature-name].md`
...
**Checkout Flow:** `$validate-test-strategy-create "checkout-flow" alto complexo --task-manager clickup --project-id 123456`  
→ ~13-14 QA points, Epic no ClickUp

**User Profile (Dry-run):** `$validate-test-strategy-create "user-profile" médio simples --dry-run`  
→ ~5-6 QA points, relatório local apenas

**Payment Integration:** `$validate-test-strategy-create "payment-integration" critico complexo --task-manager asana`  
...
- **Task Manager:** `.agents/skills/task-manager/references/`
