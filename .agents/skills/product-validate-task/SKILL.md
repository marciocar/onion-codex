...
2. **Validar a variável obrigatória do provedor ativo:**
   ...
   |----------|----------------------|----------------------------|
   | `jira` | `JIRA_HOST`, `JIRA_EMAIL`, `JIRA_API_TOKEN` | `.agents/skills/task-manager/references/adapters/jira.md` |
   | `clickup` | `CLICKUP_API_TOKEN` | `mcp_ClickUp_*` / `.agents/skills/task-manager/references/adapters/clickup.md` |
   | `asana` | `ASANA_ACCESS_TOKEN` | `mcp_asana_*` / `.agents/skills/task-manager/references/adapters/asana.md` |
   | `linear` | `LINEAR_API_KEY` | `mcp_Linear_*` / `.agents/skills/task-manager/references/adapters/linear.md` |
   | `none` / ausente | — | modo offline (sessões locais em `.codex/sessions/`) |
...
> `.agents/skills/task-manager/references/detector.md`.
...
- Analise comandos existentes em `.agents/skills/` para entender workflows
- Examine agentes especializados em `.codex/agents/` para recursos disponíveis
...
(via adapter correspondente — `.agents/skills/task-manager/references/adapters/{provedor}.md`).
...
1. **Sessão ativa**: Usa task-id do arquivo `.codex/sessions/*/context.md`
