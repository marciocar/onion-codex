...
- Lê task ID do arquivo `.codex/sessions/[slug]/context.md`
...
- **`clickup`** → comentário em formatação Unicode via `@clickup-specialist`. Adapter: `.agents/skills/task-manager/references/adapters/clickup.md`. Padrões: `docs/onion/shared/clickup-patterns.md`. Abstração MCP de referência: `commentPRUpdated()` em `.codex/utils/clickup-mcp-wrappers.md` (linhas 632-661).
...
- Confirmar task ID no arquivo `.codex/sessions/[slug]/context.md`
