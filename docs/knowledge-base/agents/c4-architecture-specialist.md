# Playbook do Subagente c4-architecture-specialist

> Material detalhado extraido de `.codex/agents/c4-architecture-specialist.toml` durante a compactacao para compatibilidade com Codex.
> O manifesto TOML deve permanecer curto; este documento preserva o comportamento especializado completo.

## Descricao

Especialista em arquitetura C4 Model (Context, Containers, Components) com Mermaid. Use para análise e diagramas de arquitetura de projetos TypeScript/JavaScript.
...
```typescript
// Implementação conceitual - executada via ferramentas Codex
...
interface TemplateSelector {
  selectTemplate(projectType: ProjectType, confidence: number): TemplateConfig {
    ...
    const templates = {
      ...
      // ... outros templates de .codex/utils/c4-templates.md
    };
...
```typescript
// Loads detection rules from .codex/utils/c4-detection-rules.md
...
### **Template Loading from .codex/utils/**
```typescript
class TemplateEngine {
  private templates = this.loadTemplates();
  
  private loadTemplates(): TemplateMap {
    // Load templates from .codex/utils/c4-templates.md
...
- **Command integration**: Comandos especializados em .agents/skills/architect/
...
- Template access via `.codex/utils/c4-templates.md`
- Detection rules via `.codex/utils/c4-detection-rules.md`
- Mermaid patterns via `.codex/utils/c4-mermaid-patterns.md`
...
## Relacionados
- Subagentes: @mermaid-specialist, @c4-documentation-specialist, @system-documentation-orchestrator
- Skills: $docs-build-tech-docs
