# Playbook do Subagente c4-documentation-specialist

> Material detalhado extraido de `.codex/agents/c4-documentation-specialist.toml` durante a compactacao para compatibilidade com Codex.
> O manifesto TOML deve permanecer curto; este documento preserva o comportamento especializado completo.

## Descricao

Especialista em documentação textual C4 Model (Context, Container, Component, ADRs). Use para documentação estruturada complementando diagramas do @c4-architecture-specialist.
...
interface CacheIntegrationEngine {
  ...
  async selectDocumentationTemplates(projectType: ProjectType): Promise<C4Templates> {
    // Carrega templates apropriados de .codex/utils/c4-documentation-templates.md
...
class C4TemplateEngine {
  ...
  private loadOfficialTemplates(): C4TemplateMap {
    // Load from .codex/utils/c4-documentation-templates.md
...
- Template access via `.codex/utils/c4-documentation-templates.md`
- Cache integration for analysis consistency

## Relacionados
- Subagentes: @c4-architecture-specialist, @mermaid-specialist, @system-documentation-orchestrator
