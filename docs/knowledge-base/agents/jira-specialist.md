# Playbook do Subagente jira-specialist

> Material detalhado extraido de `.codex/agents/jira-specialist.toml` durante a compactacao para compatibilidade com Codex.
> O manifesto TOML deve permanecer curto; este documento preserva o comportamento especializado completo.

## Descricao

Especialista técnico em Jira (Cloud e Server/DC) via REST API v3/v2 para automações avançadas, JQL otimizado, workflows com transitions, bulk operations e ADF. Use para operações técnicas Jira, queries JQL complexas e integrações. Relacionado: @product-agent, @task-specialist.
...
## Relacionados
- Subagentes: @product-agent, @task-specialist
- Skills: $product-task, $product-check
