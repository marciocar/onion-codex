...
**Agent Skills** é um formato aberto e leve para estender capacidades de agentes IA com conhecimento especializado e workflows repetíveis. Originalmente desenvolvido pela Anthropic, hoje é um padrão aberto adotado por 35+ ferramentas (Codex, GitHub Copilot, VS Code, Cursor, Gemini CLI, OpenAI Codex, OpenHands, Kiro, etc.).
...
**No Codex especificamente**:
- Skills e Commands foram **mesclados**: `.agents/skills/deploy.md` e `.agents/skills/deploy/SKILL.md` ambos criam `/deploy` e funcionam de forma equivalente
- Skills é a forma **recomendada** agora — suporta supporting files, frontmatter rico, ativação automática
- `.agents/skills/` continua funcionando (compatibilidade)
...
| Reuse entre agentes/IDEs | Skill criado no Cursor funciona no Codex |
...
Codex também varre `.agents/skills/` em **diretórios pai** até a raiz do repo, e em **diretórios filhos** sob demanda (suporte a monorepo).
...
Adotado por VS Code Copilot e outros. Alguns clientes varrem ambos `.codex/` e `.agents/` por compatibilidade.

**Regra de ouro no Sistema Onion**: usar `.agents/skills/` (é Codex-first). Para distribuir cross-client, manter cópia em `.agents/skills/` também.
...
Codex detecta mudanças **ao vivo** — adicionar/editar/remover skill em `.agents/skills/` reflete na sessão atual (sem restart, exceto se for o primeiro skill em uma pasta nova).
...
|-------|-----|-------------|
| `name` | ✅ (spec) / opcional (Codex) | Max 64 chars, lowercase + hífens, sem `--`. Codex usa nome da pasta se omitido |
| `description` | ✅ | Max 1024 chars (1536 no Codex combinado com `when_to_use`). Descreve **o quê** e **quando** |
| `license` | — | Nome ou path para arquivo de licença |
| `compatibility` | — | Max 500 chars, requisitos de ambiente |
| `metadata` | — | Map string→string |
| `allowed-tools` | — | String separada por espaços (experimental no spec; nativo no Codex) |

### Frontmatter — Extensões do Codex

Campos adicionais suportados em `.agents/skills/`:
...
⚠️ **Lifecycle no Codex**: uma vez ativado, o conteúdo do SKILL.md **fica em contexto pelo resto da sessão** — cada linha é custo recorrente de tokens. Auto-compaction preserva skills (primeiros 5000 tokens cada, budget combinado de 25000).
...
- **Cap real no Codex**: 1536 chars (`description` + `when_to_use` combinados)
...
| Limitação | Detalhe |
|-----------|---------|
| Confiança em repos externos | Skills em `.agents/skills/` de repos clonados exigem trust do workspace para `allowed-tools` aplicar |
...
### Commands ⇄ Skills (Codex)

**Importante**: no Codex, custom commands foram **mesclados** com skills.

| | `.agents/skills/deploy.md` | `.agents/skills/deploy/SKILL.md` |
|--|------------------------------|----------------------------------|
| Cria `/deploy` | ✅ | ✅ |
...
| Cross-client | ❌ Codex only | ✅ via `.agents/skills/` |

**Recomendação no Onion**:
- **Workflows novos** → criar como skill em `.agents/skills/`
- **Comandos existentes em `.agents/skills/`** → continuam funcionando, migrar quando precisar de supporting files ou ativação automática
- **Para distribuir cross-IDE** → cópia em `.agents/skills/`

### Localização recomendada no projeto

```
.agents/skills/          # ✅ Padrão Onion (Codex-first)
.agents/skills/        # ✅ Legacy / comandos puramente explícitos
.agents/skills/          # Opcional — cross-client distribution
```

### Agentes relacionados
- `@agent-skills-specialist` — criar, validar e otimizar skills
- `@command-creator-specialist` — comandos `.agents/skills/` (legacy)
- `@agent-creator-specialist` — subagents customizados
- `@codex-specialist` — config e troubleshooting Codex

---

## 🔗 Referências

- [Codex Skills (oficial)](https://code.claude.com/docs/en/skills)
...
- KBs relacionadas: [codex-commands-best-practices-2025](./codex-commands-best-practices-2025.md)

---

**Última atualização**: 2026-05-15
**Fonte primária**: https://code.claude.com/docs/en/skills (Codex) + https://agentskills.io/specification (spec aberta)
